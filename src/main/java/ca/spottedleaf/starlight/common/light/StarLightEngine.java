...
public abstract class StarLightEngine {
    ...
    // always initialised during start of lighting.
    // index = x + (z * 5)
    ...
    protected final void setupCaches(final World world, final int centerX, final int centerY, final int centerZ,
    ...
    protected final void updateVisible(final World world) {
    ...
    protected final IBlockState getBlockState(final int worldX, final int worldY, final int worldZ) {
        ...
        if (section != null) {
            ...
            return section.getData().get(worldX & 15, worldY & 15, worldZ & 15);
        }
    ...
    protected final IBlockState getBlockState(final int sectionIndex, final int localIndex) {
        ...
        if (section != null) {
            ...
            return section.getData().get(x, y, z);
        }
    ...
    public final void blocksChangedInChunk(final World world, final int chunkX, final int chunkZ,
                                           final Set<BlockPos> positions, final Boolean[] changedSections) {
        this.setupCaches(world, chunkX * 16 + 7, 128, chunkZ * 16 + 7, true, true);
        try {
            final Chunk chunk = this.getChunkInCache(chunkX, chunkZ);
            if (chunk == null) {
                return;
    ...
    protected abstract void checkBlock(final World world, final int worldX, final int worldY, final int worldZ);
    ...
    protected abstract int calculateLightValue(final World world, final int worldX, final int worldY, final int worldZ,
                                               final int expect);
    ...
    protected void checkChunkEdge(final World world, final Chunk chunk,
        ...
        for (final AxisDirection direction : ONLY_HORIZONTAL_DIRECTIONS) {
            ...
            for (int currY = chunkY << 4, maxY = currY | 15; currY <= maxY; ++currY) {
                for (int i = 0, currX = startX, currZ = startZ; i < 16; ++i, currX += incX, currZ += incZ) {
                    ...
                    if (this.calculateLightValue(world, currX, currY, currZ, currentLevel) != currentLevel) {
                        this.chunkCheckDelayedUpdatesCenter[centerDelayedChecks++] = currentIndex;
                    }

                    if (this.calculateLightValue(world, neighbourX, currY, neighbourZ, neighbourLevel) != neighbourLevel) {
    ...
    protected void checkChunkEdges(final World world, final Chunk chunk, final ShortCollection sections) {
        final int chunkX = chunk.x;
        final int chunkZ = chunk.z;

        for (final ShortIterator iterator = sections.iterator(); iterator.hasNext();) {
            this.checkChunkEdge(world, chunk, chunkX, iterator.nextShort(), chunkZ);
        }
    ...
    protected void checkChunkEdges(final World world, final Chunk chunk, final int fromSection, final int toSection) {
        final int chunkX = chunk.x;
        final int chunkZ = chunk.z;

        for (int currSectionY = toSection; currSectionY >= fromSection; --currSectionY) {
            this.checkChunkEdge(world, chunk, chunkX, currSectionY, chunkZ);
        }
    ...
    protected final void propagateNeighbourLevels(final World world, final Chunk chunk, final int fromSection, final int toSection) {
        final int chunkX = chunk.x;
        final int chunkZ = chunk.z;
                    ...
                    for (int i = 0, currX = startX, currZ = startZ; i < 16; ++i, currX += incX, currZ += incZ) {
                        ...
                        this.appendToIncreaseQueue(
                                ((currX + (currZ << 6) + (currY << (6 + 6)) + encodeOffset) & ((1L << (6 + 6 + 16)) - 1))
                                        ...
                                        // FLAG_HAS_SIDED_TRANSPARENT_BLOCKS is never set in 1.12.2, but we still
                                        // need to check opacity of the current block, so we set the flag to force the check.
    ...
    public final void forceHandleEmptySectionChanges(final World world, final Chunk chunk, final Boolean[] emptinessChanges) {
        final int chunkX = chunk.x;
        final int chunkZ = chunk.z;
        this.setupCaches(world, chunkX * 16 + 7, 128, chunkZ * 16 + 7, true, true);
        try {
            ...
            final boolean[] ret = this.handleEmptySectionChanges(world, chunk, emptinessChanges, false);
            if (ret != null) {
                this.setEmptinessMap(chunk, ret);
            }
            this.updateVisible(world);
    ...
    public final void handleEmptySectionChanges(final World world, final int chunkX, final int chunkZ,
    ...
    protected final boolean[] handleEmptySectionChanges(final World world, final Chunk chunk,
    ...
    public final void checkChunkEdges(final World world, final int chunkX, final int chunkZ) {
        this.setupCaches(world, chunkX * 16 + 7, 128, chunkZ * 16 + 7, true, false);
        try {
            final Chunk chunk = this.getChunkInCache(chunkX, chunkZ);
            if (chunk == null) {
                return;
            }
            this.checkChunkEdges(world, chunk, this.minLightSection, this.maxLightSection);
            this.updateVisible(world);
        } finally {
            this.destroyCaches();
        }
    ...
    public final void checkChunkEdges(final World world, final int chunkX, final int chunkZ, final ShortCollection sections) {
        this.setupCaches(world, chunkX * 16 + 7, 128, chunkZ * 16 + 7, true, false);
        try {
            final Chunk chunk = this.getChunkInCache(chunkX, chunkZ);
            if (chunk == null) {
            ...
            this.checkChunkEdges(world, chunk, sections);
            this.updateVisible(world);
        } finally {
            this.destroyCaches();
        }
    ...
    protected abstract void lightChunk(final World world, final Chunk chunk, final boolean needsEdgeChecks);

    public final void light(final World world, final Chunk chunk, final Boolean[] emptySections) {
        final int chunkX = chunk.x;
        final int chunkZ = chunk.z;
        this.setupCaches(world, chunkX * 16 + 7, 128, chunkZ * 16 + 7, true, true);
        ...
        try {
            ...
            // 1.12.2 fix: Pre-initialize nibbles for non-empty sections from NULL to UNINIT.
            // This prevents SkyStarLightEngine.rewriteNibbleCacheForSkylight() from nullifying them
            // in the cache, which would make sky light computation impossible when no neighbor chunks
            // are loaded (unlike modern MC, 1.12.2 does not guarantee neighbors during light init).
            final ExtendedBlockStorage[] sections = chunk.getBlockStorageArray();
            for (int i = 0; i < sections.length; ++i) {
                if (sections[i] != null && !sections[i].isEmpty()) {
                    // sections[i] = block section Y=i, nibble index = i - minLightSection = i + 1
                    nibbles[i - this.minLightSection].setNonNull();
                }
            }
            ...
            final boolean[] ret = this.handleEmptySectionChanges(world, chunk, emptySections, true);
            if (ret != null) {
                this.setEmptinessMap(chunk, ret);
            }
    ...
    public final void relightChunks(final World world, final Set<Long> chunkCoordinateSet,
        ...
        for (final long chunkCoordinate : chunkCoordinateSet) {
            ...
            for (int i = 0, len = neighbourLightOrder.length; i < len; i += 2) {
                ...
                final Chunk neighbour = world.getChunkProvider().getLoadedChunk(neighbourX, neighbourZ);
                ...
                try {
                    ...
                    for (int dz2 = -1; dz2 <= 1; ++dz2) {
                        for (int dx2 = -1; dx2 <= 1; ++dx2) {
                            ...
                            final Chunk neighbour2 = world.getChunkProvider().getLoadedChunk(neighbourX2, neighbourZ2);
                    ...
                    final boolean[] neighbourEmptiness = this.handleEmptySectionChanges(world, neighbour, getEmptySectionsForChunk(neighbour), true);
                    emptinessMapByChunk.put(key, neighbourEmptiness);
                    if (chunkCoordinateSet.contains(CoordinateUtils.getChunkKey(neighbourX, neighbourZ))) {
                        this.setEmptinessMap(neighbour, neighbourEmptiness);
                    }

                    this.lightChunk(world, neighbour, false);
                } finally {
                    this.destroyCaches();
                }
            }
            ...
            if (chunkLightCallback != null) {
                chunkLightCallback.accept(chunkCoordinate);
            }
            ++lightCalls;
        }
    ...
    // In 1.12.2, isConditionallyFullOpaque() is ALWAYS false (no VoxelShape), so this flag is never set
    // on actual blocks. However, we keep the constant for compatibility with the queue format.
    ...
    protected final void performLightIncrease(final World world) {
        ...
        while (queueReadIndex < queueLength) {
            ...
            // In 1.12.2, isConditionallyFullOpaque() is ALWAYS false, so the FLAG_HAS_SIDED_TRANSPARENT_BLOCKS
            // path (the else branch) is never meaningfully different. We keep only the simple path.
            // However, the flag can still be set on queue entries (e.g. propagateNeighbourLevels), so we
            ...
            for (final AxisDirection propagate : checkDirections) {
                ...
                if (opacityCached != -1) {
                    ...
                    // uncached opacity - need to query the world
                    this.mutablePos1.setPos(offX, offY, offZ);
                    // No VoxelShape check needed in 1.12.2
    ...
    protected final void performLightDecrease(final World world) {
        ...
        while (queueReadIndex < queueLength) {
            ...
            // In 1.12.2, isConditionallyFullOpaque() is ALWAYS false, so the FLAG_HAS_SIDED_TRANSPARENT_BLOCKS
            // path is never meaningfully different. We handle all cases with the simple path.
            ...
            for (final AxisDirection propagate : checkDirections) {
                ...
                if (opacityCached != -1) {
                    ...
                    // In 1.12.2, getLightValue is position-sensitive (Forge)
                    this.mutablePos1.setPos(offX, offY, offZ);
                    final int emittedLight = blockState.getLightValue(world, this.mutablePos1) & emittedMask;
                    if (emittedLight != 0) {
                        // re-propagate source
