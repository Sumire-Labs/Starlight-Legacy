...
public final class SkyStarLightEngine extends StarLightEngine {
    ...
    protected boolean canUseChunk(final Chunk chunk) {
        ...
        return ((ExtendedChunk)chunk).isStarlightLit() || this.isClientSide;
    }

    @Override
    protected void checkChunkEdges(final LightChunkGetter lightAccess, final Chunk chunk, final int fromSection,
                                   final int toSection) {
    ...
    protected int calculateLightValue(final LightChunkGetter lightAccess, final int worldX, final int worldY, final int worldZ,
        ...
        switch ((int)this.getKnownTransparency(worldX, worldY, worldZ)) {
            ...
            default:
                ...
                opacity = Math.max(1, state.getBlock().getLightOpacity(state, lightAccess.getWorld(), this.recalcCenterPos));
        ...
        for (final AxisDirection direction : AXIS_DIRECTIONS) {
            ...
            if (neighbourOpacity == ExtendedChunkSection.BLOCK_SPECIAL_TRANSPARENCY) {
                ...
                // Check if the neighbour blocks rendering on the face facing us (opposite direction)
                final boolean neighbourBlocksFace = neighbourState.doesSideBlockRendering(world, this.recalcNeighbourPos, direction.opposite.nms);
                // Check if we block rendering on the face facing the neighbour
                final boolean thisBlocksFace = conditionallyOpaqueState != null && conditionallyOpaqueState.doesSideBlockRendering(world, this.recalcCenterPos, direction.nms);
                if (neighbourBlocksFace || thisBlocksFace) {
    ...
    protected void propagateBlockChanges(final LightChunkGetter lightAccess, final Chunk atChunk, final Set<BlockPos> positions) {
        ...
        final World world = lightAccess.getWorld();
        final int chunkX = atChunk.x;
        final int chunkZ = atChunk.z;
    ...
    @Override
    protected void lightChunk(final LightChunkGetter lightAccess, final Chunk chunk, final boolean needsEdgeChecks) {
        this.rewriteNibbleCacheForSkylight(chunk);
        Arrays.fill(this.nullPropagationCheckCache, false);
    ...
    protected final int tryPropagateSkylight(final World world, final int worldX, int startY, final int worldZ,
        ...
        for (;startY >= (this.minLightSection << 4); --startY) {
            ...
            if (((ExtendedAbstractBlockState)above).isConditionallyFullOpaque()) {
                ...
                if (above.doesSideBlockRendering(world, this.mutablePos2, AxisDirection.NEGATIVE_Y.nms)) {
            ...
            if (opacityIfCached != -1) {
                ...
                if (((ExtendedAbstractBlockState)current).isConditionallyFullOpaque()) {
                    // Check if the current block blocks rendering on the face facing up (towards the source)
                    if (current.doesSideBlockRendering(world, mutablePos, AxisDirection.POSITIVE_Y.nms)) {
                        // can't propagate here, we're done on this column.
                        break;
                    }

                    // Also check if the above block blocked on the way down - already handled above for
                    // conditionally opaque 'above', so here we just need to check the current block's own face
                    flags |= FLAG_HAS_SIDED_TRANSPARENT_BLOCKS;
                }

                final int opacity = current.getBlock().getLightOpacity(current, world, mutablePos);
                if (opacity > 0) {
