...
public final class SkyStarLightEngine extends StarLightEngine {
        ...
        if (currentLevel == 15) {
            ...
            this.appendToIncreaseQueue(
                    ((worldX + (worldZ << 6) + (worldY << (6 + 6)) + encodeOffset) & ((1L << (6 + 6 + 16)) - 1))
                            ...
                            // no FLAG_HAS_SIDED_TRANSPARENT_BLOCKS in 1.12.2 (no VoxelShape)
            );
        } else {
    ...
    protected int calculateLightValue(final World lightAccess, final int worldX, final int worldY, final int worldZ,
        ...
        if (opacity < 0) {
            ...
            opacity = Math.max(1, centerState.getLightOpacity(lightAccess, this.recalcCenterPos));
    ...
    protected final int tryPropagateSkylight(final World world, final int worldX, int startY, final int worldZ,
        ...
        for (;startY >= (this.minLightSection << 4); --startY) {
            ...
            // 1.12.2: no VoxelShape / conditionallyOpaque checks on the 'above' block
            // isConditionallyFullOpaque is always false, so we skip the fromShape logic entirely
            ...
            if (opacityIfCached != -1) {
                ...
                // no conditional opacity / VoxelShape checks in 1.12.2
