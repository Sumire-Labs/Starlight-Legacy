...
public abstract class StarLightEngine {
    ...
    protected final void setupCaches(final LightChunkGetter chunkProvider, final int centerX, final int centerY, final int centerZ,
        ...
        for (int dz = -radius; dz <= radius; ++dz) {
            for (int dx = -radius; dx <= radius; ++dx) {
                ...
                final Chunk chunk = chunkProvider.getChunkForLighting(cx, cz);
    ...
    protected final void updateVisible(final LightChunkGetter lightAccess) {
        final int ySections = (this.maxLightSection - this.minLightSection + 1) + 2; // includes buffer sections
        for (int index = 0, max = this.nibbleCache.length; index < max; ++index) {
            ...
            if ((nibble != null && nibble.updateVisible()) || this.notifyUpdateCache[index]) {
                if (this.isClientSide) {
                    // Decode section position from cache index
                    final int chunkX = (index % 5) - this.chunkOffsetX;
                    final int chunkZ = ((index / 5) % 5) - this.chunkOffsetZ;
                    final int chunkY = ((index / (5 * 5)) % ySections) - this.chunkOffsetY;

                    // Notify renderer to re-render this section
                    lightAccess.getWorld().markBlockRangeForRenderUpdate(
    ...
    protected final IBlockState getBlockState(final int worldX, final int worldY, final int worldZ) {
        ...
        if (section != null) {
            return section == EMPTY_CHUNK_SECTION ? AIR_BLOCK_STATE : section.getData().get(worldX & 15, worldY & 15, worldZ & 15);
        }
    ...
    protected final IBlockState getBlockState(final int sectionIndex, final int localIndex) {
        ...
        if (section != null) {
            ...
            return section == EMPTY_CHUNK_SECTION ? AIR_BLOCK_STATE : section.getData().get(localIndex & 15, (localIndex >> 8) & 15, (localIndex >> 4) & 15);
        }
    ...
    protected abstract void propagateBlockChanges(final LightChunkGetter lightAccess, final Chunk atChunk, final Set<BlockPos> positions);
    ...
    protected void checkChunkEdge(final LightChunkGetter lightAccess, final Chunk chunk,
    ...
    protected void checkChunkEdges(final LightChunkGetter lightAccess, final Chunk chunk, final ShortCollection sections) {
    ...
    protected void checkChunkEdges(final LightChunkGetter lightAccess, final Chunk chunk, final int fromSection, final int toSection) {
        final int chunkX = chunk.x;
        final int chunkZ = chunk.z;
    ...
    protected final void propagateNeighbourLevels(final LightChunkGetter lightAccess, final Chunk chunk, final int fromSection, final int toSection) {
        final int chunkX = chunk.x;
        final int chunkZ = chunk.z;
    ...
    public static Boolean[] getEmptySectionsForChunk(final Chunk chunk) {
        ...
        for (int i = 0; i < sections.length; ++i) {
            if (sections[i] == null || sections[i] == Chunk.NULL_BLOCK_STORAGE || sections[i].isEmpty()) {
                ret[i] = Boolean.TRUE;
            } else {
                ret[i] = Boolean.FALSE;
            }
    ...
    public final void forceHandleEmptySectionChanges(final LightChunkGetter lightAccess, final Chunk chunk, final Boolean[] emptinessChanges) {
        final int chunkX = chunk.x;
        final int chunkZ = chunk.z;
        this.setupCaches(lightAccess, chunkX * 16 + 7, 128, chunkZ * 16 + 7, true, true);
        try {
    ...
    protected final boolean[] handleEmptySectionChanges(final LightChunkGetter lightAccess, final Chunk chunk,
    ...
    protected abstract void lightChunk(final LightChunkGetter lightAccess, final Chunk chunk, final boolean needsEdgeChecks);

    public final void light(final LightChunkGetter lightAccess, final Chunk chunk, final Boolean[] emptySections) {
        final int chunkX = chunk.x;
        final int chunkZ = chunk.z;
        this.setupCaches(lightAccess, chunkX * 16 + 7, 128, chunkZ * 16 + 7, true, true);
        ...
        while (queueReadIndex < queueLength) {
            ...
            if ((queueValue & FLAG_HAS_SIDED_TRANSPARENT_BLOCKS) == 0L) {
                ...
                for (final AxisDirection propagate : checkDirections) {
                    ...
                    if (opacityCached != -1) {
                        ...
                        if (((ExtendedAbstractBlockState)blockState).isConditionallyFullOpaque()) {
                            if (blockState.doesSideBlockRendering(world, this.mutablePos1, propagate.getOpposite().nms)) {
                                continue;
                            }
                            flags |= FLAG_HAS_SIDED_TRANSPARENT_BLOCKS;
                        }

                        final int opacity = blockState.getBlock().getLightOpacity(blockState, world, this.mutablePos1);
                ...
                for (final AxisDirection propagate : checkDirections) {
                    ...
                    // Check if the from block occludes in the propagation direction
                    final boolean fromOccludes = ((ExtendedAbstractBlockState)fromBlock).isConditionallyFullOpaque()
                            && fromBlock.doesSideBlockRendering(world, this.mutablePos2, propagate.nms);
